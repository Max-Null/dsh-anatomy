 ...
 * dsh-anatomy 图书馆构建器
 *
 * **派生自** `seek-soul-in-darkness/shell/scripts/build-decision-index.mjs`（SSiD 决策知识库）。
 * 展示层（PAGE_CSS / PAGE_JS，含极简 markdown 渲染器与那个已修的死循环防护）**原样复用** ——
 * 那是被 111 篇真实文档与 37 项断言验证过的部分，重写只会把踩过的坑再踩一遍。
 *
 * 与母版的两处**实质差异**：
 *   1. **递归扫描 + 目录即分类**：本库按 `版本对比/决策精读/工程范式/` 分组，而母版是平铺 + 状态归一
 *   2. **没有状态维度**：研究笔记没有"已完成/进行中"这种生命周期，强行套用会造出不存在的语义
 *      （分类 chip 也顺手改成从数据动态生成，去掉了母版里那处硬编码）
 *
...
/** 库的元信息文件不当作研究笔记，单独归一组 */
const META = new Set(['README.md', '基线.md']);

/** 目录名即分类；根目录下的归「库说明」 */
function scan(dir, group, out) {
  for (const e of fs.readdirSync(dir, { withFileTypes: true })) {
    if (e.name.startsWith('.')) continue;
    const p = path.join(dir, e.name);
    if (e.isDirectory()) { scan(p, group === null ? e.name : group, out); continue; }
    if (!e.name.endsWith('.md')) continue;
    out.push({ file: path.relative(ROOT, p).replace(/\\/g, '/'), abs: p, group: group ?? (META.has(e.name) ? '库说明' : '未分组') });
  }
}
...
function extractTags(text) {
  ...
  for (const m of text.matchAll(/v?(\d+\.\d+\.\d+(?:-[a-z0-9.]+)?)/g)) t.add(m[1]);
  return [...t].slice(0, 14);
}

function build() {
  const found = [];
  scan(ROOT, null, found);
  const entries = [];
  for (const f of found) {
    const text = fs.readFileSync(f.abs, 'utf8');
    const lines = text.split('\n');
    const fnDate = path.basename(f.file).match(/^(\d{4}-\d{2}-\d{2})-/)?.[1] ?? '';
    const h1 = lines.find((l) => /^#\s+\S/.test(l));
    const title = h1 ? h1.replace(/^#\s+/, '').trim()
      : path.basename(f.file).replace(/^\d{4}-\d{2}-\d{2}-/, '').replace(/\.md$/, '');
    entries.push({
      file: f.file, group: f.group, date: fnDate,
      slug: path.basename(f.file).replace(/^\d{4}-\d{2}-\d{2}-/, '').replace(/\.md$/, ''),
      title, tags: extractTags(text), summary: extractSummary(lines.slice(1).join('\n')),
      bytes: Buffer.byteLength(text, 'utf8'), lines: lines.length, body: text,
    });
  }
...
const PAGE_JS = [
  ...
  '  });',
  '  var ORDER=["版本对比","决策精读","工程范式","库说明"];var sc=[];ORDER.forEach(function(g){if(bySt[g])sc.push([g,bySt[g]]);});Object.keys(bySt).forEach(function(g){if(ORDER.indexOf(g)<0)sc.push([g,bySt[g]]);});',
...
function buildHtml(data) {
  ...
  return [
    ...
    '<title>dsh-anatomy · DSH 解剖库</title>',
...
console.log('  总体积      ' + (data.totalBytes / 1024 / 1024).toFixed(2) + ' MB');
console.log('  分类：');
for (const [g, n] of Object.entries(byGroup).sort((a, b) => b[1] - a[1])) {
  console.log('    ' + String(n).padStart(3) + '  ' + g);
}
console.log('  \n  ✓ 写出 ' + path.relative(ROOT, OUT_JSON));

if (!process.argv.includes('--json-only')) {
  fs.writeFileSync(OUT_HTML, buildHtml(data), 'utf8');
  console.log('  ✓ 写出 ' + path.relative(ROOT, OUT_HTML) + '  (' + (fs.statSync(OUT_HTML).size / 1024 / 1024).toFixed(2) + ' MB)');
}
console.log('');
